...
def sync_tencent_token_to_cloudflare(token_value):
    """把腾讯文档 token 同步到 Cloudflare KV，供备用服务读取"""
    cf_token = CF_API_TOKEN
    if not cf_token:
    ...
    try:
        ...
        if resp.status_code == 200 and data.get("success"):
            return True, ""
        return False, f"KV 写入失败 {resp.status_code}: {resp.text[:200]}"
    except Exception as e:
        return False, f"KV 同步异常: {e}"
...
def admin_update():
    ...
    return jsonify({
        "success": True,
        "effective": True,
        "storage": "render_env+cloudflare_pages" if key == "TENCENT_ACCESS_TOKEN" else "render_env",
